...
menu.action(menu.my_root(), "taxi!", {}, "", function(_)
    local sc = Scaleforms.findInstance("PLAYER_SWITCH")
    Scaleforms.activate(sc, 15000)
    while Scaleforms.isActive(sc) do
        Scaleforms.runMethod(sc, "renderDisplay", 0)
        Scaleforms.runMethod(sc, "SET_SWITCH_VISIBLE", true)
        Scaleforms.runMethod(sc, "SET_SWITCH_HINTED", 0, true)
        util.yield()
    end
end)
...
menu.action(menu.my_root(), "heist.exe", {}, "", function(_)
    local sc = Scaleforms.create("HEIST_MP")
    Scaleforms.runMethod(sc, "CLEAR_VIEW")
    Scaleforms.runMethod(sc, "ADD_CREW_MEMBER", 0, "Jackz", 1000, "Hacker", 1, 0, "Ready", 0, 10000000, 100, 0, "?", "Pro", 4)
    Scaleforms.runMethod(sc, "ADD_CREW_MEMBER", 1, "Heavenira", 101, 2, "Gunner", 0, "Ready", 0, 1000, -10, 0, "?", "Ugly", 4)
    Scaleforms.runMethod(sc, "ADD_CREW_MEMBER", 2, "Ezra", 101, 3, 1, 0, "Canadian", 0, 1000, 0, 0, "?", "Ugly", 4)
    Scaleforms.runMethod(sc, "BLANK_CREW_MEMBER", 3)
    Scaleforms.runMethod(sc, "SET_HEIST_NAME", "Hacking Rockstar")
    Scaleforms.runMethod(sc, "SET_LEADER_COST", "$1,000,000")
    ...
    Scaleforms.runMethod(sc, "INITIALISE_HEISTBOARD")
    Scaleforms.runMethod(sc, "SHOW_HEISTBOARD")
    Scaleforms.runMethod(sc, "ADD_LAUNCH_BUTTON")
    Scaleforms.activate(sc, 3500)
end)
menu.action(menu.my_root(), "orbital", {}, "", function(_)
    local sc = Scaleforms.create("ORBITAL_CANNON_CAM")
    local i = 0
    Scaleforms.runMethod(sc, "SET_STATE", 1) --5
    local c = math.random(10, 100)
    local items = 0
    while i <= 10000 do
        HUD.HIDE_HUD_AND_RADAR_THIS_FRAME()
        Scaleforms.displayFullscreen(sc, 255, 255, 255, 255)
        if i % 10 == 0 then
            Scaleforms.runMethod(sc, "SET_MENU_TITLE", generate_hack_string(math.random(1, 20)))
            Scaleforms.runMethod(sc, "SET_MENU_HELP_TEXT", generate_hack_string(math.random(1, 20)))
            Scaleforms.runMethod(sc, "SET_CHARGING_LEVEL", math.random(1, 20))
            Scaleforms.runMethod(sc, "SET_ZOOM_LEVEL", math.random(1, 7))
            Scaleforms.runMethod(sc, "SET_COUNTDOWN", c)
            c = c - 1
            if c < 0 then
                c = math.random(10, 100)
            end
            for id = 1, items do
                Scaleforms.runMethod(sc, "REMOVE_MENU_ITEM", id)
            end
            for id = 1, items do
                Scaleforms.runMethod(sc, "ADD_MENU_ITEM", id, generate_hack_string(math.random(1, 20)), generate_hack_string(math.random(1, 20)), true, math.random() > 0.5)
            end
        end
        if i % 100 == 0 then
            items = math.random(1, 10)
        end
        util.yield()
        i = i + 1
    end
    Scaleforms.deactivate(sc)
end)
local droneMeters = {
    "RETICLE",
    "ZOOM_METER",
...
menu.action(menu.my_root(), "drone", {}, "", function(_)
    local sc = Scaleforms.create("DRONE_CAM")
    local my_ped = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(players.user())
    local my_pos = ENTITY.GET_ENTITY_COORDS(my_ped, true)
    local my_pos2 = ENTITY.GET_ENTITY_COORDS(my_ped, true)
    my_pos2.x = my_pos2.x + 1.0
    local i = 0
    while i <= 10000 do
        HUD.HIDE_HUD_AND_RADAR_THIS_FRAME()
        Scaleforms.displayFullscreen(sc, 255, 255, 255, 255)
        if i % 10 == 0 then
            for _, meter in ipairs(droneMeters) do
                Scaleforms.startMethod(sc, "SET_" .. meter .. "_IS_VISIBLE")
                Scaleforms.addBool(math.random() > 0.123)
                Scaleforms.endMethod()
            end
            for _, perc in ipairs(dronePercs) do
                Scaleforms.startMethod(sc, "SET_" .. perc .. "_PERCENTAGE")
                Scaleforms.addFloat(math.random())
                Scaleforms.endMethod()
            end
        Scaleforms.startMethod(sc, "SET_INFO_LIST_DATA")
        for _  = 1, 4 do
            Scaleforms.addFloat(math.random(-1000000, 1000000))
        end
        Scaleforms.addString(generate_hack_string(math.random(0,25)))
        Scaleforms.addFloat(math.random(0, 100))
        Scaleforms.addString(generate_hack_string(math.random(0,25)))
        Scaleforms.addString(generate_hack_string(math.random(0,25)))
        Scaleforms.addFloat(math.random(-1000000, 1000000))
        Scaleforms.addFloat(math.random(-1000000, 1000000))
        Scaleforms.addString(generate_hack_string(math.random(0,35)))
        Scaleforms.endMethod()

        Scaleforms.runMethod(sc, "SET_ZOOM", math.random(0, 7))
        Scaleforms.runMethod(sc, "SET_RETICLE_STATE", math.random(0, 5))
        Scaleforms.runMethod(sc, "SET_HEADING", math.random(0, 360))

        Scaleforms.startMethod(sc, "SET_RETICLE_ON_TARGET")
        Scaleforms.addBool(math.random() > 0.8)
        Scaleforms.endMethod()
    end
...
menu.action(menu.my_root(), "fuck up my atm", {}, "", function(_)
    for _ = 1, 10 do
        Scaleforms.builtin.atm.setMousePos(math.random(), math.random())
        util.yield(500)
    end
    Scaleforms.builtin.atm.display("Heavenira", "$", -1000.0)
end)
menu.action(menu.my_root(), "phone", {}, "", function(_)
    local sc = Scaleforms.create("CELLPHONE_IFRUIT")
    Scaleforms.endMethod()
    Scaleforms.startMethod(sc, "SET_TITLEBAR_TIME")
    Scaleforms.addInt(math.random(0, 24))
    Scaleforms.addInt(math.random(0, 60))
    Scaleforms.addInt(math.random(0, 12))
    Scaleforms.endMethod()

    Scaleforms.startMethod(sc, "SET_THEME")
    Scaleforms.addFloat(1)
    Scaleforms.endMethod()

    Scaleforms.startMethod(sc, "SET_SLEEP_MODE")
    Scaleforms.addBool(false)
    Scaleforms.endMethod()

    Scaleforms.startMethod(sc, "SET_BACKGROUND_IMAGE")
    Scaleforms.addFloat(2)
    Scaleforms.endMethod()
    
    
    -- Scaleforms.activate(sc)
    local i = 0
    local a1 = {
        x = math.random(),
        y = math.random(),
        w = math.random(0, 50) / 100.0,
        h = math.random(0, 50) / 100.0,
    }
    local a2 = {
        x = math.random(),
        y = math.random(),
    ...
    local size_x = 1
    local size_y = 1
    local size_x_inc = math.random() > 0.5 and 1.0 or -1.0
    local size_y_inc = math.random() > 0.5 and 1.0 or -1.0

    while i <= 10000 do
        Scaleforms.display_3d(sc, my_pos, { x = 0.0, y = 0.0, z = 50 }, { 50.0, 100.0, 1.5}, 50.0)
        -- Scaleforms.display_3d_solid(sc, my_pos2, { x = 0.0, y = 0.0, z = h }, { }, 50.0)
        -- Scaleforms.display_3d_solid(sc, my_pos2, { x = 0.0, y = 0.0, z = h }, { x = size_x, y = size_y }, 50.0)
        -- Scaleforms.display_3d_solid(sc, my_pos, { x = size_x, y = size_y, z = h }, { x = 1.0, y = 1.0 }, 50.0)
        -- Scaleforms.display_3d_solid(sc, my_pos, { x = 0, y = 0, z = 0 }, { x = size_x, y = size_y }, 50.0)

        h = h + h_i
        size_x = size_x + size_x_inc
        size_y = size_y + size_y_inc
        if h == 180 then
            h_i = -5
        elseif h == 0 then
            h_i = 1
        end
        if size_x > math.random(0, 5) or size_x <= 0 then
            size_x_inc = -size_x_inc
        elseif size_y > math.random(0, 5) or size_y <= 0 then
            size_y_inc = -size_y_inc
        end
        -- Scaleforms.display(sc, a1.x, a1.y, a1.w, a1.h, { r = 255, g = 255, b = 255, a = 255})
        -- Scaleforms.display(sc, a2.x, a2.y, a2.w, a2.h, { r = 255, g = 255, b = 255, a = 255})

        -- Scaleforms.display(sc, math.random(), math.random(), math.random(), math.random(), { r = 255, g = 255, b = 255, a = 255})
        Scaleforms.startMethod(sc, "SET_SIGNAL_STRENGTH")
        Scaleforms.addInt(math.random(0, 5))
        Scaleforms.endMethod()

        Scaleforms.startMethod(sc, "SET_HEADER")
        Scaleforms.addString(generate_hack_string(16))
        Scaleforms.endMethod()
...
menu.action(menu.my_root(), "aircraft2", {}, "", function(_)
    local sc = Scaleforms.Scaleform:create("AIRCRAFT_DIALS")
    sc:run("SET_DASHBOARD_LIGHTS", false, true, true)
    local sc2 =  Scaleforms.Scaleform:create("CELLPHONE_ALERT_POPUP")
    sc2:run("CREATE_ALERT", 2, 0, 0, "test")
    sc:activate(5000)
    sc2:activate(10000)
    
end)
menu.action(menu.my_root(), "aircraft", {}, "", function(_)
    local sc = Scaleforms.create("AIRCRAFT_DIALS")
    Scaleforms.startMethod(sc, "SET_DASHBOARD_DIALS")
    for x = 1, 12 do
        Scaleforms.addInt(math.random())
    end
    Scaleforms.endMethod()
    -- Scaleforms.call_method_with_numbers(sc, "SET_DASHBOARD_DIALS", math.random(), math.random(), math.random(), math.random(), math.random(), math.random(), math.random(), math.random(), math.random(), math.random(), math.random(), math.random())

    Scaleforms.startMethod(sc, "SET_DASHBOARD_LIGHTS")
    Scaleforms.addBool(true)
    Scaleforms.addBool(true)
    Scaleforms.addBool(true)
    Scaleforms.endMethod()

    -- Scaleforms.call_method_with_strings(sc, "SET_AIRCRAFT_HUD", "TEST1", "TEST2", "TESt3", "test4")
    Scaleforms.activate(sc, 10000)
end)
menu.action(menu.my_root(), "news", {}, "", function(_)
    if scaleform > 0 then
        GRAPHICS.SET_SCALEFORM_MOVIE_AS_NO_LONGER_NEEDED(scaleform)
    end
    -- local a = Scaleforms.builtin.ShowBreakingNewsScrolling("OPRESSORS ARE COMING", "Law makers are proposing that fish should be made illegal too", { "Applesauce is now illegal in 5 states.", "Line 2 message goes here" }, 15000)
    local topLines = {
        "A",
        "B",
        "C",
    ...
    local a = Scaleforms.builtin.showBreakingNewsScrolling("titlebar", topLines, btmLines, 15000, 5000)
...
menu.action(menu.my_root(), "text", {}, "", function(_)
    ...
    Scaleforms.builtin.showHackingMessage("!!!!", "OPRESSORS ARE COMING", 15000, 255, 0, 255)
end)
